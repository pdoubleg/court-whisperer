...
def find_fuzzy_matches_in_docs(
    ...
    best_matches = process.extract(
        query,
        [d.page_content for d in docs_clean],
        limit=k,
    ...
    for i, m in enumerate(real_matches):
        for j, doc_clean in enumerate(docs_clean):
            if m in doc_clean.page_content:
    ...
    for match in orig_doc_matches:
        choice_text = match.page_content
        contexts = []
        while choice_text != "":
...
def find_closest_matches_with_bm25(
    ...
    if len(docs) == 0:
        return []
    texts = [doc.page_content for doc in docs_clean]
...
def eliminate_near_duplicates(passages: List[str], threshold: float = 0.9) -> List[str]:
    """
